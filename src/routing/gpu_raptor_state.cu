...
stats_{std::vector<gpu_raptor_stats>(32)},tmp_{std::vector<gpu_delta_t>(n_locations)},best_{std::vector<gpu_delta_t>(n_locations)},station_mark_{std::vector<uint32_t>(n_locations)},
