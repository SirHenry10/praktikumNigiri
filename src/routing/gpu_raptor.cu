...
__device__ gpu_transport get_earliest_transport(unsigned const k,
  ...
  for (auto i = gpu_day_idx_t::value_t{0U}; i != n_days_to_iterate; ++i) {
    auto const ev_time_range =
        gpu_it_range{i == 0U ? seek_first_day() : get_begin_it(event_times),
    ...
    for (auto it = begin(ev_time_range); it != end(ev_time_range); ++it) {
      ...
      if (is_better_or_eq<SearchDir>(time_at_dest_[k],
                          to_delta(day, ev_mam) + dir<SearchDir>(lb_[gpu_to_idx(l)]))) {
  ...
  *any_station_marked_ = false; // aktualisieren hiervon kein problem beim Parallelisieren -> wenn es einmal true ist bleibt es auch true
  ...
  for (auto i = 0U; i != stop_seq.size(); ++i) {
    ...
    if (et.is_valid() && ((SearchDir == gpu_direction::kForward) ? stp.out_allowed() : stp.in_allowed())) {
      ...
      assert(by_transport != std::numeric_limits<gpu_delta_t>::min() &&
             by_transport != std::numeric_limits<gpu_delta_t>::max());
    ...
    if (is_better_or_eq<SearchDir>(prev_round_time, et_time_at_stop)) {
      ...
      auto const new_et = get_earliest_transport<SearchDir, Rt>(k, r, stop_idx, day, mam,
                                                 stp.gpu_location_idx(), stats_, lb_, time_at_dest_, route_transport_ranges, n_days_);
      current_best =
    ...
    if(!marked(route_mark_, r_idx)) {
      ...
      if(get_global_thread_id() == 0){
        update_route<SearchDir, Rt>(k, r, route_location_seq, stats_, prev_station_mark_, best_, round_times_,
                                    ...
                                    base_, route_transport_ranges, n_days_, route_stop_times);
      }
      this_grid().sync();
  }
...
__device__ void update_transfers(unsigned const k, bool const * is_dest_, uint16_t* dist_to_end_,
  ...
  for(auto l_idx = global_t_id;
    ...
    if(is_better<SearchDir>(fp_target_time, best_[l_idx])
      ...
      printf("update_arrivals %d ", (k-1) * row_count_round_times_ + l_idx);
      bool updated = update_arrival(round_times_, (k-1) * row_count_round_times_ + l_idx, fp_target_time);
...
__device__ void update_footpaths(unsigned const k, gpu_profile_idx_t const prf_idx, unsigned short kUnreachable,
  ...
  for(auto idx = global_t_id;
    ...
    for(auto const& fp: fps){
      ...
      printf("update_arrivals %d ", (k-1) * row_count_round_times_ + gpu_to_idx(gpu_location_idx_t{fp.target_}));
      bool updated = update_arrival(round_times_,(k-1) * row_count_round_times_ + gpu_to_idx(gpu_location_idx_t{fp.target_}), fp_target_time);
  ...
  for(auto idx = global_t_id;
    ...
    if((marked(prev_station_mark_, idx) || marked(station_mark_, idx)) && dist_to_end_[idx] != kUnreachable){
      ...
      if(is_better<SearchDir>(end_time, gpu_kIntermodalTarget[(*best_)].v_)){
        printf("update_arrivals %d ", (k-1) * row_count_round_times_ + gpu_kIntermodalTarget->v_);
        bool updated = update_arrival(round_times_, (k-1) * row_count_round_times_ + gpu_kIntermodalTarget->v_, end_time);
...
__device__ void raptor_round(unsigned const k, gpu_profile_idx_t const prf_idx,
  ...
  for(auto idx = global_t_id; idx < n_locations; idx += global_stride){
    auto test =round_times_[(k-1) * row_count_round_times_ +idx];
