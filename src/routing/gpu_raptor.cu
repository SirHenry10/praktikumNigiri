...
template <gpu_direction SearchDir, bool Rt>
__device__ void raptor_round(unsigned const k, gpu_profile_idx_t const prf_idx, gpu_raptor<SearchDir,Rt>& gr){
  ...
  for(auto idx = global_t_id; idx < *gr.gtt_.n_locations_; idx += global_stride){
    ...
    if(gr.is_dest_[idx]){
      update_time_at_dest(k, gr.mem_->device_.best_[global_t_id], gr);
    }
  }
  this_grid().sync();
